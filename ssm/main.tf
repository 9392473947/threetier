locals {
  type = var.type != null ? var.type : (
    length(var.values) > 0 ? "StringList" : (
      can(tostring(var.value)) ? (var.secure_type ? "SecureString" : "String") : "StringList"
  ))
...
resource "aws_ssm_parameter" "this" {
  ...
  value          = local.secure_type || local.string_type ? local.value : null
  insecure_value = local.list_type ? local.value : null
